  ...
  return (
    <section className={`py-20 pb-10 ${gradient || 'bg-gradient-to-b from-indigo-500/[.04] to-transparent dark:from-indigo-900/20'}`}>
      <div className="max-w-4xl mx-auto px-6 text-center">
        ...
        <h1 className="text-4xl md:text-5xl lg:text-[3.5rem] font-extrabold tracking-tight text-slate-900 dark:text-white leading-[1.1] mb-5">{title}</h1>
        ...
        {(primaryCta || secondaryCta) && (
          <div className="flex flex-wrap justify-center gap-3 mt-7">
            ...
            {secondaryCta && <Link href={secondaryCta.href} className="px-5 py-3 rounded-xl text-sm font-semibold border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-white hover:border-wz-primary hover:text-wz-primary transition">{secondaryCta.label}</Link>}
          </div>
        )}
      </div>
    </section>
